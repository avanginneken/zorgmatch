...
export default async function BeheerLayout({
  ...
  if (!isSupabaseConfigured()) {
    ...
    try {
      ...
      if (session.rol !== 'BEHEER') redirect('/inloggen')
      return (
        <div className="min-h-screen bg-gray-50">
          <Navbar rol="BEHEER" naam={session.naam} />
          <div className="flex">
            <Sidebar rol="BEHEER" />
            <main className="flex-1 p-6 min-w-0"><Breadcrumb />{children}</main>
          </div>
        </div>
      )
    } catch {
  ...
  if (!gebruiker || gebruiker.rol !== 'BEHEER') {
    redirect('/inloggen')
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar rol="BEHEER" naam={gebruiker.naam} />
      <div className="flex">
        <Sidebar rol="BEHEER" />
